...
def community_led_details(excel_file):
    try:
        ...
        expected_columns = TABS_METADATA["COMMUNITY_LED_PROGRAMS"]
        ...
        for row_idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
            try:
                row_data = {
                    'label': row[cleaned_headers.index(TABS_METADATA["COMMUNITY_LED_PROGRAMS"][0])] or '',
                    'value': row_data['value'] + row[cleaned_headers.index(TABS_METADATA["COMMUNITY_LED_PROGRAMS"][1])] or '',
                }
        ...
        if not found:
            json_data.append({'type': 'pie-chart', 'data': data})
