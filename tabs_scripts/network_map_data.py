...
def get_coordinates(state, country, partner_id=None, partners_data=None):
    ...
    if partner_id and partners_data:
        for p in partners_data:
            if p.get("partner_id", "").lower() == str(partner_id).lower():
                coords = p.get("coords")
...
#                 source_coords = get_coordinates(source_state, source_country, partner_id , partner_data)
#                 if source_coords:
    ...
    try:
        ...
        for row_idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
            try:
                ...
                source_coords = get_coordinates(source_state, source_country, source_partner, partner_data)
                if source_coords:
                    source["coords"] = source_coords
                else:
                    print(f"⚠️ Skipping row {row_idx} due to missing source coords.")
