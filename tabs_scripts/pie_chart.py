...
def pie_chart(excel_file):
    try:
        ...
        try:
            sheet = workbook[PAGE_METADATA["DASHBOARD_FIRST_PAGE"]]
        except KeyError:
        ...
        expected_columns = TABS_METADATA["PIE_CHART"]
        ...
        for row_idx, row in enumerate(sheet.iter_rows(min_row=8, values_only=True), start=2):
            try:
                row_data = {
                    ...
                    'value': row[cleaned_headers.index(TABS_METADATA["PIE_CHART"][2])] or '',
                }
        ...
        # Find and update the "pie-chart" object
        found = False
